...
void OptimizedThreadPool::push_task(FuncType f, int* array, const std::size_t size) {
    // вычисляем индекс очереди, куда положим задачу
    int queue_to_push = m_qindex++ % m_thread_count;
    task_type new_task([=] {f(array, size); });//создается функтор с заданной функцией, переданной через лямду
...
void RequestHandler2::push_task(FuncType f, int* array, const std::size_t size) {
    m_tpool.push_task(f, array, size);
}
